...
<main class="relative z-10 flex-1 px-4 sm:px-6 md:px-8 font-karla font-semibold">
    ...
    <!-- Top Bar -->
    <header style="background-color: #1E40AF;" class="flex flex-col sm:flex-row justify-between items-center bg-blue-800 text-white px-4 sm:px-6 py-4 rounded-lg shadow-md space-y-2 sm:space-y-0">
                            ...
                            <tr class="border-b border-gray-300 text-center hidden sm:table-row"> 
                                ...
                                <td class="py-3 px-4">
                                    <span class="px-3 py-1 rounded text-white text-xs sm:text-sm 
                                        {{ $schedule->status == 'accepted' ? 'bg-green-500' : ($schedule->status == 'declined' ? 'bg-red-500' : 'bg-orange-500') }}">
